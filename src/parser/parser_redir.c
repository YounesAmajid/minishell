...
/*   Updated: 2023/09/28 19:37:43 by yamajid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_line(char *line, char *delimiter)
{
...
bool	parse_redir(t_redir **redir, t_token **tokens, t_env *env)
{
	...
	if (node->type == ROUT || node->type == APPEND)
		node->file_flages |= O_CREAT | O_WRONLY | ((node->type == ROUT) * 
				O_TRUNC + !(node->type == ROUT) * O_APPEND);
